    ...
    public fun run() {
        ...
        while (true) {
            ...
            thread {
                ...
                val bufferReaderProcessor = BufferedReader(InputStreamReader(processorMicroservice.inputStream))
...
//                            val message = processorMicroservice.getInputStream().read().toString()
                            val message = bufferReaderProcessor.readLine()
                            println("Am primit de la procesor: $message")
                            if(message != "")
                                broadcastMessage("intrebare ${clientConnection.port} $message", except = clientConnection.port)

                        }
                        "raspuns" -> {
                            // tipul mesajului de tip raspuns este de forma:
                            // raspuns <CONTINUT_RASPUNS>
                            processorMicroservice.getOutputStream().write("raspuns ${clientConnection.port} $messageBody\n".toByteArray())
//                            val message = processorMicroservice.getInputStream().read().toString()
                            val message = bufferReaderProcessor.readLine()
                            println("Am primit de la procesor2: $message")
                            if(message != "")
                                respondTo(messageDestination.toInt(), message)
                        }
                    }
                }
            }
