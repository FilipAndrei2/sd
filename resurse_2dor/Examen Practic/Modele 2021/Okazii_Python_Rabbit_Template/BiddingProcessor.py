...
class BiddingProcessor:
    def __init__(self):
        # Rezultatul licitatiei va fi salvat in fisier
        self.producer = open("result.txt", "w")
        # Va consuma elementele din coada biddingprocessor
    ...
    def get_processed_bids(self):
        ...
        # Timp de asteptare procesare auction
        time.sleep(30)

        # Ofertele se stocheaza sub forma de perechi <id:<ID>_amount:<SUMA>>
        bids = dict()

        # Se intercepteaza mesaje in coada
        try:
        ...
        while len(self.consumer.list_msg) != 0:
            # Se preia mesajul din coada
            msg = self.consumer.list_msg.pop()

            # Daca este de tip incheiat, terminam bucla
            if msg == "incheiat":
                break

            # Impartim mesajul si l stocam
            msg = msg.split("_")
            bids[msg[0].split(":")[1]] = msg[1].split(":")[1]

            # Asteptam interceptare mesaje noi
            try:
                self.consumer.receive_message()
            except Exception as e:
                print("except:{}".format(e))
    ...
    def decide_auction_winner(self, bids):
        ...
        # Cazul in care nu sunt oferte procesate
        if len(bids) == 0:
